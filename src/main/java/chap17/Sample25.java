...
public class Sample25 {
    ...
    public static void main(String[] args){
        ...
        scores.stream().collect(Collectors.minBy(Comparator.comparing(Score::getName))).ifPresent(System.out::println);

        System.out.println("** 이름순으로 제일 느린 학생");
        scores.stream().collect(Collectors.maxBy(Comparator.comparing(Score::getName))).ifPresent(System.out::println);
    }
}
